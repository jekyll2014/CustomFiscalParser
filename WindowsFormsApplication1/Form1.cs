...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            defaultCSVToolStripTextBox.Text = CustomFiscalParser.Properties.Settings.Default.CommandsDatabaseFile;
            errorsCSV_toolStripTextBox.Text = CustomFiscalParser.Properties.Settings.Default.ErrorsDatabaseFile;
            ...
            ReadCsv(CustomFiscalParser.Properties.Settings.Default.ErrorsDatabaseFile, ErrorsDatabase);
        }
        ...
        public void ReadCsv(string fileName, DataTable table)
        {
            ...
            while (c != '\r' && c != '\n' && c != -1)
            {
                byte[] b = new byte[1];
                b[0] = (byte)c;
                inputStr.Append(Encoding.GetEncoding(CustomFiscalParser.Properties.Settings.Default.CodePage).GetString(b));
            ...
            {
                string[] cells = inputStr.ToString().Split(CustomFiscalParser.Properties.Settings.Default.CSVdelimiter);
            ...
            while (c != -1)
            {
                ...
                while (i < colNum && c != -1 /*&& c != '\r' && c != '\n'*/)
                {
                    ...
                    byte[] b = new byte[1];
                    b[0] = (byte)c;
                    if (c == CustomFiscalParser.Properties.Settings.Default.CSVdelimiter) i++;
                    if (c != -1) inputStr.Append(Encoding.GetEncoding(CustomFiscalParser.Properties.Settings.Default.CodePage).GetString(b));
                }
                while (c != '\r' && c != '\n' && c != -1) c = inputFile.ReadByte();
                if (inputStr.ToString().Replace(CustomFiscalParser.Properties.Settings.Default.CSVdelimiter, ' ').Trim().TrimStart('\r').TrimStart('\n').TrimEnd('\n').TrimEnd('\r') != "")
                {
                    string[] cells = inputStr.ToString().Split(CustomFiscalParser.Properties.Settings.Default.CSVdelimiter);
        ...
        private void Button_find_Click(object sender, EventArgs e)
        {
            ...
            if (ParseEscPos.FindCommand(textBox_code.SelectionStart))
            {
                ...
                textBox_code.Select(textBox_code.SelectionStart, ParseEscPos.commandBlockLength);
            }
        ...
        {
            saveFileDialog.FileName = CustomFiscalParser.Properties.Settings.Default.CommandsDatabaseFile;
        ...
        private void SaveFileDialog_FileOk(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                File.WriteAllText(saveFileDialog.FileName, textBox_code.Text, Encoding.GetEncoding(CustomFiscalParser.Properties.Settings.Default.CodePage));
            }
            ...
            else if (saveFileDialog.Title == "Save CSV database")
            {
                ...
                try
                {
                    File.WriteAllText(saveFileDialog.FileName, output.ToString(), Encoding.GetEncoding(CustomFiscalParser.Properties.Settings.Default.CodePage));
                }
                catch (Exception ex)
                {
        ...
        {
            if (defaultCSVToolStripTextBox.Text != CustomFiscalParser.Properties.Settings.Default.CommandsDatabaseFile)
            {
                CustomFiscalParser.Properties.Settings.Default.CommandsDatabaseFile = defaultCSVToolStripTextBox.Text;
                CustomFiscalParser.Properties.Settings.Default.Save();
            }
        }
        ...
        {
            if (errorsCSV_toolStripTextBox.Text != CustomFiscalParser.Properties.Settings.Default.ErrorsDatabaseFile)
            {
                CustomFiscalParser.Properties.Settings.Default.ErrorsDatabaseFile = errorsCSV_toolStripTextBox.Text;
                CustomFiscalParser.Properties.Settings.Default.Save();
            }
        }
    }
